...
for i = 1:size(review_array, 2)
    fprintf('Parsing %d/%d ', i, size(review_array,2));%%%%
    ...
    for j = 1:num_words
        ...
        stop_word_flag = 0;%%%%%%%no stop words
        ...
        if( ~strcmp(prev_word, ' ') && ~strcmp(prev_word, '') && ~strcmp(word, ' ') && ~strcmp(word, ''))
            ...
            bigram = char(strcat(prev_word,{' '}, word)); %create the bigram
            % add bigram to map
...
for i = 1:size(review_array, 2)
    fprintf('Vectorizing %d/%d ', i, size(review_array,2));%%%%
    review = review_array{i}{1};
    review = parse_review_string(review);
    review_words = strsplit(review, ' ');
    stemmed_review = [];
    
    for j = 1:size(review_words, 2)%turn review to stemmed review
        word = review_words(j);
        word = porterStemmer(cell2mat(word));
        % add stemmed word
        if (j==1)
